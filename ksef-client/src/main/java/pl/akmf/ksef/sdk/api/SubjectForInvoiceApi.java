...
public class SubjectForInvoiceApi {
    ...
    public SubjectForInvoiceApi(HttpApiClient apiClient) {
     ...
     */
    public ApiResponse<PermissionsOperationResponse> apiV2PermissionsEntitiesGrantsPost(GrantEntityPermissionsRequest grantEntityPermissionsRequest, String authenticationToken) throws ApiException {
        Map<String, String> headers = new HashMap<>();
        headers.put(AUTHORIZATION, BEARER + authenticationToken);
        headers.put(CONTENT_TYPE, APPLICATION_JSON);
        headers.put(ACCEPT, APPLICATION_JSON);

        var response = apiClient.post(GRANT_INVOICE_SUBJECT_PERMISSION.getUrl(), grantEntityPermissionsRequest, headers);

        if (response.statusCode() / 100 != 2) {
            throw getApiException(GRANT_INVOICE_SUBJECT_PERMISSION.getOperationId(), response.body(), response.statusCode(), response.headers());
        }
