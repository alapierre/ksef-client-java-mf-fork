...
public class GrantDirectlyApi {
    ...
    public GrantDirectlyApi(HttpApiClient apiClient) {
     ...
     */
    public ApiResponse<PermissionsOperationResponse> apiV2PermissionsIndirectGrantsPost(IndirectPermissionsGrantRequest indirectPermissionsGrantRequest, String authenticationToken) throws ApiException {
        Map<String, String> headers = new HashMap<>();
        headers.put(AUTHORIZATION, BEARER + authenticationToken);
        headers.put(CONTENT_TYPE, APPLICATION_JSON);
        headers.put(ACCEPT, APPLICATION_JSON);

        var response = apiClient.post(GRANT_INDIRECT_PERMISSION.getUrl(), indirectPermissionsGrantRequest, headers);

        if (response.statusCode() / 100 != 2) {
            throw getApiException(GRANT_INDIRECT_PERMISSION.getOperationId(), response.body(), response.statusCode(), response.headers());
        }
